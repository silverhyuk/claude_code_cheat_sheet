...
[공식 체인지로그](https://code.claude.com/docs/en/changelog)를 매일 스크래핑하여 새로운 릴리스가 있으면 `index.html`을 자동으로 업데이트하고, GitHub에 커밋/푸시합니다.
...
```
체인지로그에서 릴리스 정보를 파싱할 수 없습니다
```

→ 공식 사이트 구조가 변경되었을 수 있음. `auto_update.py`의 `parse_changelog()` 함수 수정 필요
