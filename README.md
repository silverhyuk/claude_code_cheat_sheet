...
```mermaid
flowchart TD
    A[🕐 crontab<br/>매일 1회] --> B[run_update.sh<br/>환경 세팅]
    B --> C[auto_update.py<br/>메인 로직]
    C --> D[(VERSION.md<br/>현재 버전)]
    C --> E[(CHANGELOG.md<br/>GitHub raw)]
    D --> F{새 버전 있는가?}
    E --> F
    F -- No --> Z[종료]
    F -- Yes --> G[OpenAI API<br/>gpt-5.3-codex<br/>추가 항목 JSON 추출]
    G --> H[index.html에<br/>항목별 코드 삽입<br/>원본 구조 유지]
    H --> I[index.html<br/>항목 추가]
    H --> J[VERSION.md 갱신<br/>버전 + 히스토리]
    I --> K[git commit & push]
    J --> K
```
